...
function billableAmount(ms, rate) {
  ...
  // toFixed before rounding: hours * rate * 100 can land a hair under an exact
  // half-cent in binary (0.03 * 7.5 * 100 is 22.499999999999996), and plain
  // Math.round would then round 0.225 down to 0.22 instead of up to 0.23. Measured at
  // 3.3% of realistic minute/rate combinations before this fix.
  return Math.round((billableHours(ms) * r * 100).toFixed(6)) / 100;
}
